...
def go(model_name, epochs=50, inputs='./log/*.jpg', limit=None):
    ...
    callbacks = [
        keras.callbacks.EarlyStopping(monitor='val_loss', patience=conf.training_patience, verbose=0),
        keras.callbacks.ModelCheckpoint(filepath=model_name, monitor='val_loss', save_best_only=True, verbose=0),
    ]
    ...
    # Save the model
    model_save_path = model_name if model_name.endswith('.keras') or model_name.endswith('.h5') else f"{model_name}.keras"
    model.save(model_save_path)  # No need for save_format argument
    print(f"Model saved at {model_save_path}")
    #print('loading the model...')
    #model = keras.models.load_model(model_save_path, custom_objects={'tf': tf}, safe_mode=False)
    #print('loaded')
    ...
    try:
        ...
        print(f"problems with loss graph: {e}")
