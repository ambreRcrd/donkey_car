...
def go(filename, address, constant_throttle=0, num_cars=1, image_cb=None, rand_seed=None):
    model_path = filename  # Use the filename provided in the arguments
    print("loading model", model_path)

    model = None

    try:
        # Ensure the correct method is used for loading the model
        if model_path.endswith('.keras') or model_path.endswith('.h5'):
            ...
            raise ValueError("Unsupported model file format. Expected .keras or .h5 extension.")
    ...
    try:
        ...
        print(f"An error occurred while compiling the model: {e}")
        return
...
#        raise FileNotFoundError(f"File not found: {model_path}. Please ensure the file is an accessible `.keras` or SavedModel directory.")
#    
#    with open(model_path, 'r') as f:
#        model_config = json.load(f)
#
#    try:
#        model = tf.keras.models.model_from_config(model_config, custom_objects={'safe_mode': False})
#        print("Model loaded successfully.")
#        return model
#    except Exception as e:
#        raise RuntimeError(f"An error occurred while loading the model: {e}")
#
#def go(model_path, address, constant_throttle, num_cars=1, rand_seed=None):
#    model_path = '/Users/ambrericouard/sdsandbox/outputs/mymodel.keras'
...
#        return
#
#    # Additional logic for predictions can be added here
