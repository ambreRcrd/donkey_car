...
plotter = Plotter()
#mpc = MPCController(plotter, N=10, dt=0.05, L=0.08)
mpc = MPCController(N=10, dt=0.05, L=0.08)
traj_logger = TrajectoryLogger()

# Initial state [x, y, theta, v]
X0 = np.array([80, 60, 0, 0.05, 0, 50])
X0 = np.array([80, 60, 0])

ref = np.zeros((4, mpc.N+1))
ref[0, :] = np.linspace(0, 1, mpc.N+1)  # x reference
ref[1, :] = 1  # y reference (straight line)
ref[2, :] = 0  # theta reference (straight direction)
ref[3, :] = 0.05  # v reference (constant speed)

def drive(cfg, model_path=None, use_joystick=False, model_type=None,
          camera_type='single', meta=[]):
    global steering_values, throttle_values, errors, Y_values, theta_values, Y, theta, throttle, psi, v, xd_prev_val, yd_prev_val, xd_prev_prev_val, yd_prev_prev_val, distance_beteween_contours, i_error, p_error, d_error, steering, throttle, mpc, X0, ref, plotter, traj_logger
    """
    ...
    def process_image_and_control(image):
        global mpc, plotter, X0, steering, traj_logger

        image_processor = ImageProcessor(image, plotter, mpc)
        xd, yd, dxd, dyd, ddxd, ddyd, theta = image_processor.process_image()

        #dxd, dyd, ddxd, ddyd = 0, 0, 0, 0
        #throttle, steering = mpc.compute_control(X0, xd, yd, theta, 10) #, dxd, dyd, ddxd, ddyd, theta)
        #print(f"Time step : Throttle = {throttle:.3f}, Steering = {steering:.3f}")

        if abs(theta) < 0.1:
            throttle, steering = mpc.compute_control(X0, segment='straight')
        elif theta > 0:
            throttle, steering = mpc.compute_control(X0, segment='curved_left')
        elif theta < 0:
            throttle, steering = mpc.compute_control(X0, segment='curved_right')

        X0 = mpc.dynamics(X0, [throttle, steering]) #, xd, yd, dxd, dyd)
        X0 = np.array(X0.full()).flatten()

        print('X0 : ', X0)
        traj_logger.add_trajectory_point(X0[0], X0[1], X0[2], xd, yd, dxd, dyd, ddxd, ddyd, steering, throttle)
        traj_logger.save_to_json()

        #X0 = np.array([X0[0], X0[1], X0[2], throttle, steering, X0[5]])
        #X0 = np.array([X0[0], X0[1], X0[2]])

        print(f"State at time step : X = {X0[0]:.3f}, Y = {X0[1]:.3f}, Theta = {X0[2]:.3f}")

        return steering, throttle


        # Lambda part to process the images and find line position
    V.add(Lambda(process_image_and_control), inputs=['cam/image_array'], outputs=['user/angle', 'user/throttle'])
    ...
    def plot_trajectory_from_json(json_file):
        # Charger les données du fichier JSON
        with open(json_file, 'r') as f:
            data = json.load(f)
        
        # Extraire les données de la trajectoire
        x_car = [point['x'] for point in data['trajectory']]
        y_car = [point['y'] for point in data['trajectory']]

        # Modèle de référence - circuit
        plt.figure(figsize=(10, 6))
        plt.gca().set_facecolor('lightgray')

        # Inner figure (Orange, thicker lines)
        x1 = np.linspace(-3, 3, 400)
        y1 = np.full_like(x1, -2)
        plt.plot(x1, y1, 'orange', linewidth=3)

        y2 = np.full_like(x1, 2)
        plt.plot(x1, y2, 'orange', linewidth=3)
        ...
        # Outer figure (Orange, thicker lines with same distance from inner)
        x1_outer = np.linspace(-3, 3, 400)
        y1_outer = np.full_like(x1, -4)
        plt.plot(x1_outer, y1_outer, 'orange', linewidth=3)

        y2_outer = np.full_like(x1, 4)
        plt.plot(x1_outer, y2_outer, 'orange', linewidth=3)

        theta = np.linspace(0, np.pi, 400)
        y3_outer = 4 * np.cos(theta)
        x3_outer = 3 + 4 * np.sin(theta)
        plt.plot(x3_outer, y3_outer, 'orange', linewidth=3)

        theta = np.linspace(np.pi, 2*np.pi, 400)
        y4_outer = 4 * np.cos(theta)
        x4_outer = -3 + 4 * np.sin(theta)
        plt.plot(x4_outer, y4_outer, 'orange', linewidth=3)

        # Intermediate figure (White, dashed lines)
        x1_mid = (x1 + x1_outer) / 2
        y1_mid = (y1 + y1_outer) / 2
        plt.plot(x1_mid, y1_mid, 'white', linewidth=2, linestyle='--')

        y2_mid = (y2 + y2_outer) / 2
        plt.plot(x1_mid, y2_mid, 'white', linewidth=2, linestyle='--')

        x3_mid = (x3 + x3_outer) / 2
        y3_mid = (y3 + y3_outer) / 2
        plt.plot(x3_mid, y3_mid, 'white', linewidth=2, linestyle='--')

        x4_mid = (x4 + x4_outer) / 2
        y4_mid = (y4 + y4_outer) / 2
        plt.plot(x4_mid, y4_mid, 'white', linewidth=2, linestyle='--')

        # Tracer la trajectoire du véhicule à partir des données JSON
        plt.plot(x_car, y_car, 'red', linewidth=2, label='Car Trajectory')

        # Configuration des limites de l'axe
        plt.xlim(-10, 10)
        plt.ylim(-10, 10)

        # Définir le ratio des axes pour être égaux
        ...
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.title('Car Trajectory on Circuit (Data from JSON)')
    ...
    def exit_handler():
        plot_trajectory_from_json('trajectory_data.json')
        #plotter = Plotter()
        #pass
